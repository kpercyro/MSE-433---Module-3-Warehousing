...
def main(seed=100, outdir='.'):
    ...
    # Multi-restart SA with small random perturbations to params
    candidates = []
    sa_configs = [
        (42, 'total_completion_time', 20000, 100, 0.9997),
        (43, 'makespan',              20000, 100, 0.9997),
        (7,  'total_completion_time', 25000, 120, 0.99975),
        (13, 'makespan',             25000,  80, 0.99960),
        (99, 'total_completion_time', 22000, 110, 0.99968),
    ]
    for sa_seed, obj, iters, T0, alpha in sa_configs:
        random.seed(sa_seed)
        sa = SA(sim, demands, obj, iters=iters, T0=T0, alpha=alpha)
        sol, _ = sa.run()
        res = sim.simulate(sol)
        candidates.append((sol, res, f"{obj}(s={sa_seed},T={T0},a={alpha})"))

    opt_sol, opt_res, chosen = min(candidates, key=lambda x: x[1]['makespan'])
    print(f"\n  Selected: {chosen}")
